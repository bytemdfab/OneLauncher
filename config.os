...
Функция Отладка(Знач URLОтладчика = Неопределено) Экспорт
...
Функция Хранилище(Знач ПутьКХранилищу, Знач ИмяПользователя, Знач Пароль) Экспорт
	
	мПараметрыЗапуска.Вставить("Хранилище", Истина);
	мПараметрыЗапуска.Вставить("ПутьКХранилищу", мПуть.Составить(ПутьКХранилищу));
	мПараметрыЗапуска.Вставить("ИмяПользователяХранилища", ИмяПользователя);
	мПараметрыЗапуска.Вставить("ПарольХранилища", Пароль);
...
Функция КоманднаяСтрока() Экспорт
	...
	Если мПараметрыЗапуска.Свойство("ПутьКПлатформе") Тогда
	...
	Если мПараметрыЗапуска.Свойство("Предприятие") И мПараметрыЗапуска.Предприятие Тогда
		...
		Если мПараметрыЗапуска.Свойство("Отладка") И мПараметрыЗапуска.Отладка Тогда
	...
	ИначеЕсли мПараметрыЗапуска.Свойство("Конфигуратор") И мПараметрыЗапуска.Конфигуратор Тогда
		...
		Если мПараметрыЗапуска.Свойство("Хранилище") И мПараметрыЗапуска.Хранилище Тогда
			Состав.Добавить("/ConfigurationRepositoryF "+мПараметрыЗапуска.ПутьКХранилищу);
			Состав.Добавить("/ConfigurationRepositoryN "+мПараметрыЗапуска.ИмяПользователяХранилища);
			Состав.Добавить("/ConfigurationRepositoryP "+мПараметрыЗапуска.ПарольХранилища);
		КонецЕсли;
